...
export async function GET(request: NextRequest) {
  try {
    ...
    const articlesResponse = await fetch(`${API_BASE_URL}/api/articles?status=published&limit=50&sort=-publishedAt`);
    ...
    // Fetch categories for additional category tags
    ...
    const siteConfig: SiteConfig = {
      ...
      description: 'Latest news, breaking stories, and in-depth coverage of current events',
    ...
    // Generate RSS XML
    const rssXml = generateRSSFeed(articles, categories, siteConfig);
...
function generateRSSFeed(articles: Article[], categories: Category[], siteConfig: SiteConfig): string {
  const now = new Date();
  ...
  const generateCategoryTags = (article: Article, categories: Category[]): string => {
    ...
    categories.forEach(category => {
      if (category.name.toLowerCase() !== article.category?.name?.toLowerCase()) {
        const categoryNameLower = category.name.toLowerCase();
        if (contentLower.includes(categoryNameLower) || titleLower.includes(categoryNameLower)) {
          tags.push(`<category><![CDATA[${category.name}]]></category>`);
        }
      }
    });
  ...
  const rssItems = articles.map(article => {
    ...
    const categoryTags = generateCategoryTags(article, categories);
...
1	</sy:updateFrequency>
	<generator>${siteConfig.url}/?v=1.0.0</generator>
